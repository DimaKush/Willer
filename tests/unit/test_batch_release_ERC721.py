...
def test_batch_release_ERC721_releasable_false(add_will, extend_release_time, willer_contract, beneficiaries, testator, ERC721_token_contracts, n_ids):
    with brownie.reverts("Willer: unreleasable"):
        willer_contract.batchReleaseERC721(testator, list(ERC721_token_contracts), [list(range(n_ids))] * len(ERC721_token_contracts), {'from': beneficiaries[0]})


def test_batch_release_ERC721_releasable_true(add_will, wait_release_time, willer_contract, beneficiaries, testator, ERC721_token_contracts, n_ids):
    expected = {}
