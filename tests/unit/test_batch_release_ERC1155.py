...
def test_batch_release_ERC1155_releasable_false(add_will, extend_release_time, willer_contract, testator, beneficiaries, ERC1155_token_contracts, n_ids):
    for ERC1155_token_contract in ERC1155_token_contracts:
        with brownie.reverts("Willer: unreleasable"):
            willer_contract.batchReleaseERC1155(testator, ERC1155_token_contract, list(range(n_ids)), {'from': beneficiaries[0]})


def test_batch_release_ERC1155_releasable_true(add_will, wait_release_time, willer_contract, testator, beneficiaries, shares, ERC1155_token_contracts, n_ids):
    expected = {}
    for ERC1155_token_contract in ERC1155_token_contracts:
